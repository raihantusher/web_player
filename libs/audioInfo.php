...
class AudioInfo {
	...
	*/
	...
	public function Info($file) {
		...
		// Init wrapper object
		$this->result = array();
		$this->result["filesize"]=(isset($this->info["filesize"])?$this->info["filesize"]:"");
...
class RTinfo extends AudioInfo {
	...
	private function playtime($duration){
		$duration=round($duration);
		if($duration>60)
		{
				return floor($duration/60).":".ceil($duration%60);
		}
		return '00:'.$duration;
	}

	private function bitrate($bitrate){
		return ($bitrate/1000)." Kbits/s";
	}

	private function size($byte){
			$unit="KB";

			$size=round($byte/1024);

			if(($size/1024)%1024!=0){
				$unit= "MB";
				//$size=ceil($size/1024).'.'.ceil($size%1024);
